...
#la.house.data
la.house.data$garage = ifelse(is.na(la.house.data$garage),0,la.house.data$garage)  
#When I was doing the homework I checked to see that the command was successful via  'la.house.data$garage'
...
*Because I think the type of housing makes a huge difference in the price (perhaps an interaction variable somewhere?), I definitely need to deal with the 39 missing values for the categorical variable 'type'.*
...
*Model 1 regresses the log of price on the interaction term log of square footage * type (alternative, townhome/condo, sing family residence), bedrooms, bathrooms, if a house has a pool, and the number of garage bays.* 
...
*For instance, the eqquation would look like this:*
...
1. Each additional 100(?) feet in square footage can increase the price by 1.445139 or 144%.  
2. Each additional bedroom reduces the price for the same square footage by 10.6% (-0.105772).  
3. Each additional bathroom increases the price for the same square footage by 4.6% (0.046606).  
4. A pool can increase the prices of a house of equivalent square footage by 28% (0.280487).  
5. Each additional garage bay reduces the price of the house with equivalent square footage by 14.6% (-0.146483).  
