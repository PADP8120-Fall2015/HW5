...
#Find all the missing values for eaach variable.
sapply(la.house.data, function(x) sum(is.na(x)))
```
...
-Residuals are 'iid'; identically & independently distributed (errors do no affect each other).
-Nothing about Beta1 affects Beta2, ..., BetaK.
-Linear Realtionship

*Let's test for iid residuals.*
```{r}
