...
###### Formulas
1. Our model is: __$\mathbf{Y} = \mathbf{X} \mathbf{\beta} + \mathbf{\epsilon}$__
...
victoria.matrix.lm = function(outcome.matrix,design.matrix){betas = solve(t(x.vars) %*% x.vars) %*% t(x.vars) %*% outcome.matrix
betas = round(betas,5)
